...
function main() {
  ...
  try {
    ...
    assert.match(fireProtocolText, /For transient capacity, do not increment any retry, interruption, clarification, implementation, or Saver bound/)
    assert.match(fireProtocolText, /30 seconds, 60 seconds, 120 seconds, and 300 seconds/)
    assert.match(fireProtocolText, /at most two same-attempt non-capacity interruption restarts/)
    assert.match(fireProtocolText, /P2\/P3, `FOLLOWUP`, and `INVALID` findings are advisory and never block integration/)
    assert.match(fireProtocolText, /Allow at most five substantive normal Implementation attempts per generation, and track review passes separately/)
    assert.match(fireProtocolText, /first evidence-complete review in a generation uses `DISCOVERY`.*regardless of the Implementation attempt ordinal/)
