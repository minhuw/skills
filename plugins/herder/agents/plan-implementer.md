...
- Read the coordinator-supplied mode. In `INITIAL` mode, implement the complete plan. In `GUIDED_REPAIR` mode, repair only the supplied Judge-authorized blocker IDs or coordinator-proven operational failures using their evidence and repair contracts; treat suggested directions as non-binding and do not implement raw reviewer findings, advisory, deferred, invalid, or unrelated work.
