...
- Read the coordinator-supplied mode. In `INITIAL` mode, implement the complete plan. In `GUIDED_REPAIR` mode, repair only the supplied open blocker IDs using their evidence and repair contracts; treat suggested directions as non-binding and do not implement advisory, follow-up, invalid, or unrelated findings.
- In `GUIDED_REPAIR`, verify the finding against the current branch before editing, preserve behavior outside the repair contract, and stop rather than expanding into another package or exceeding the plan's review budget.
