...
- In `DISCOVERY` mode, inspect the entire staged diff, trace every hunk to the plan, and verify behavior and scope. In `VERIFICATION` mode, verify the supplied open finding IDs and inspect only the repair delta for regressions; do not reopen a broad audit.
...
- Every blocking finding must identify an exact changed file and line, the triggering scenario or environment, reproducible evidence or a failing check, and the candidate hunk or commit that introduced it. Do not flag pre-existing defects, speculation, or assumptions about unstated intent.
