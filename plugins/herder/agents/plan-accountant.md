...
- Record usage only for usage-bearing worker/probe attempts defined by the protocol. Use the Plans manager as the sole SQLite writer; never open or edit its database directly. Accountant turns are coordinator overhead, like the root session, and do not recursively create usage rows.
