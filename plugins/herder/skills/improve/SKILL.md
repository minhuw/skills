...
Create one or more focused plans per selected finding using the complete shared template. Every plan declares `Kind`, `Parent objective`, a numeric `files<=N` expected review target, `## Dependency contract`, and `## Review map`. Fire adds only a fixed three-file adjudicated contingency for directly necessary implementation-discovered companions inside the same bounded subsystem; it is not permission to omit credible paths during planning or add a public transition. Put only verified facts repeated by multiple plans in `herder-plans/CONTEXT.md`; compiled snapshots, not sibling files or the audit transcript, must supply complete executor context. Update the index with order, dependencies, and status, preserving the manager-owned execution-usage block and markers verbatim.
