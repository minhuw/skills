...
> Herder run; the persistent Accountant owns status transitions.
>
...
- [ ] All done criteria above pass; the Herder Accountant owns status updates
...
Written by a Herder plan producer and updated only by the persistent Herder Accountant during execution:
...
STOP conditions; only the persistent Herder Accountant updates status rows.
