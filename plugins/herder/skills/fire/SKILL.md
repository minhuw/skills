...
- Treat the effective parallel limit as one global role-agnostic pool for Implementer, Reviewer, Judge, and Saver attempts. Keep exactly one restack/gate/review/Judge/integration transaction lane across the plan set and reserve one pool slot while that lane is claimed. Other slots must remain eligible for independent Implementer, guided-repair Implementer, or Saver work on different plans; never wait for an implementation wave to drain before beginning review.
...
- Give each plan generation at most five substantive normal Implementation attempts. Track review passes separately: the first evidence-complete review is bounded discovery regardless of attempt number, and every later review is targeted verification. Route ordinary implementation, restacking, gate, and reconciliation failures through a fresh Implementer in the same plan worktree while the implementation budget remains. Proven clean host interruptions are free.
- Dispatch Judge after every Reviewer response, including approval. Judge alone decides which findings are blocking and in scope, which are nonblocking, which are invalid, and which valid out-of-scope findings must be written as non-executable drafts under `plan_dir/leak/`. Pass only Judge-authorized repair contracts to Implementer.
- Treat `files<=N` as the expected target, with a fixed three-file contingency and hard ceiling `N+3`. A directly necessary implementation-discovered companion path may proceed to review only inside the declared bounded subsystem, without an unplanned public transition or unordered-plan overlap, and with Implementer justification plus explicit Reviewer and Judge acceptance. More than three discovered paths or any hard-ceiling/hard-scope violation stops for replan before broad review.
- Treat Saver as one optional fresh recovery implementation, not a general retry loop. Dispatch it only after all five normal Implementation attempts are exhausted, Judge confirms the original task is incomplete, and actionable in-scope blockers remain. Give Saver the immutable task, current branch evidence, failed attempts, and only Judge-authorized blockers. After its one substantive attempt, rerun gates, Reviewer, and Judge; never dispatch a second substantive Saver attempt for that generation.
- Distinguish agent attempts from substantive implementation and Saver attempts. Record every host-interrupted attempt, but do not consume a mutation budget when the protocol proves no response or worktree mutation occurred. Confirmed transient capacity uses a fresh session with backoff and consumes no substantive budget; bound other same-attempt interruption restarts separately.
...
- Make review convergence coordinator-owned: only Judge-confirmed, evidence-complete original-plan failures and patch-introduced P0/P1 regressions block integration; P2/P3 and invalid findings remain advisory, while valid unrelated findings go to `plan_dir/leak/`. Keep a stable relationship ledger, permit exactly one broad discovery, use targeted verification thereafter, and invoke Judge after every review.
