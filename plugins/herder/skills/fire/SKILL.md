...
- For continuous visual observation, direct the user to `$herder:dashboard`; it independently polls README, SQLite, and Git and never invokes Accountant or changes Fire state.
...
Never add `plans/execution.yaml`, another scheduler-state file, another accounting database, or another plan parser. The Plans manager's `.herder/execution.sqlite3` is the sole attempt-accounting store and is not scheduler truth.
...
- Preserve the user's branch, index, source changes, and untracked files. Plans lifecycle/accounting updates and Judge-retained deferred findings under `plan_dir/leak/` are the only coordination-checkout writes.
...
- Keep ordinary assignment verification branch-bound. A detached HEAD is admissible only through the protocol's explicit active-rebase mode for one ownerless conflicted rebase, one `GUIDED_REPAIR` Implementer, a remaining round, exact durable worktree/branch/assignment/rebase/checkpoint/lease evidence, and a sealed unchanged-state hash. Never auto-detect this exception or alter Git state to make it pass; after the worker, require a completed rebase, clean attached plan branch, and ordinary verification again.
- Create checkpoint refs only from `coordination-ref.mjs format-checkpoint`, using canonical `generation-<n>-<zero-padded-ordinal>` suffixes. Namespace, rebase, and cleanup readers share its parser, accept legacy numeric-generation suffixes read-only, and reject every unknown coordination ref without renaming or rewriting evidence.
