...
`)
  ...
  assert.match(protocol, /one event-driven global worker pool/)
  ...
  assert.match(protocol, /Coordinator gates, restacks, and the integration lock consume no worker slot/)
  assert.match(protocol, /Fill every available worker slot from eligible Implementer, Reviewer, and Judge actions/)
  ...
  assert.match(protocol, /integration lock is a coordinator mutex, not a worker and not a pool reservation/)
