...
try {
  ...
  assert.match(credentialValidation.stderr, /contains unknown fields: probe/)
...
`)
  ...
  assert.equal(Object.hasOwn(preflightEvidence, "probes"), false)
  assert.match(preflightEvidence.executables["plan-implementer"], /fake-bin\/grok$/)
  ...
  assert.match(fireSkill, /--profile <name>/)
  assert.match(fireSkill, /eclipse.*shannon/)
  assert.match(fireSkill, /profile-registry.*resolve --host <codex\|claude>/s)
  assert.match(fireSkill, /frontier|selected profile|selected-profile/i)
  assert.match(fireSkill, /never derive names, substitute a generic agent, fall back to a default/)
  assert.match(fireSkill, /pre-handle rejection consumes no attempt or round|before it returns a worker handle.*not an attempt or round/s)
  assert.match(fireSkill, /Only the Accountant may invoke mutating control-plane helper modes/)
  assert.match(fireSkill, /coordination-ref\.mjs/)
  assert.match(fireSkill, /canonical `generation-<n>-<zero-padded-ordinal>`/)
  assert.match(fireSkill, /accept legacy numeric-generation suffixes read-only/)
  ...
  assert.match(fireSkill, /explicit active-rebase mode/)
  assert.match(fireSkill, /Never auto-detect this exception/)
  ...
  assert.match(protocol, /Accountant is the exclusive mutating control-plane owner/)
  ...
  assert.match(protocol, /selected_profile.*profile name, SHA-256, host, required root-orchestrator model\/effort, and exact five-role child mapping/)
  assert.match(protocol, /bind-profile/)
  assert.match(protocol, /read the immutable profile binding before spawning the Accountant/)
  assert.match(protocol, /never silently adopts a changed default/i)
  assert.match(protocol, /rejection before a worker handle exists.*consumes no attempt or round/i)
  assert.match(protocol, /never retry through another profile, model, or generic agent/i)
  ...
  assert.match(protocol, /inspect-active-rebase/)
  assert.match(protocol, /verify --verification-mode active-rebase --expected-rebase-state-sha256/)
  assert.match(protocol, /A detached HEAD by itself is never recovery authority/)
  assert.match(protocol, /rebase metadata to be absent, the exact plan branch to be attached, the worktree to be clean/)
  ...
  assert.match(protocol, /node <coordination_ref> format-checkpoint/)
  assert.match(protocol, /generation-1-001/)
  assert.match(protocol, /writers never emit or rewrite that legacy form/)
  assert.match(protocol, /Every other checkpoint suffix remains unknown coordination state and fails closed/)
  ...
  assert.match(orcaProtocol, /does not query model catalogs or spend tokens on model probes/)
  ...
  for (const profile of [codexAccountant, claudeAccountant]) {
    ...
    assert.match(profile, /do not treat detached .*HEAD.* as recovery authority/)
    assert.match(profile, /inspect-active-rebase.*active-rebase .*verify/s)
    assert.match(profile, /ordinary branch-bound assignment verification/)
  }
