...
`)
  ...
  assert.match(fireSkill, /Default parallel limit: `5`/)
  assert.match(fireSkill, /--max-parallel <n>.*positive integer/)
  assert.match(fireSkill, /global role-agnostic pool for Implementer, Reviewer, Judge, and Saver attempts/)
  assert.match(fireSkill, /reserve one pool slot while that lane is claimed/)
  assert.match(fireSkill, /never wait for an implementation wave to drain before beginning review/)

  const protocol = await readFile(path.join(scriptDir, "..", "references", "orchestration-protocol.md"), "utf8")
  assert.match(protocol, /parallel_limit.*otherwise `5`.*capped by host-available worker capacity/)
  assert.match(protocol, /one event-driven, role-agnostic worker pool for the whole run/)
  assert.match(protocol, /one claimed transaction lane reserves one parallel slot/i)
  assert.match(protocol, /immediately backfill eligible global slots before waiting again/)
  assert.match(protocol, /Never wait for all implementations in a wave to finish before starting the first review transaction/)
  assert.match(protocol, /one plan may use the reserved lane for Reviewer or Judge while up to four different plans run Implementer, guided-repair Implementer, or Saver attempts/)
  assert.match(protocol, /release the transaction-lane reservation and dispatch a fresh `plan_implementer` in `GUIDED_REPAIR` mode through the global pool/)
  assert.match(protocol, /Saver may overlap unrelated mutation attempts and another plan's single review transaction/)
  assert.match(protocol, /queue the branch to reacquire the transaction lane/)
  assert.match(protocol, /Reconstruct `scheduler_state`; never store it as a second source of truth/)
  ...
  assert.match(protocol, /every command—including the first bundle hash\/type check—to set that exact assigned worktree as its explicit workdir/)
  ...
  assert.doesNotMatch(protocol, /sum added plus deleted lines/)
  assert.doesNotMatch(protocol, /file\/line overflow/)
  ...
  assert.match(protocol, /exact current Git, gate, and rebase evidence/)
  assert.match(protocol, /compact summaries of all five normal attempts and every completed review\/Judge outcome/)
  assert.match(protocol, /P2\/P3, `FOLLOWUP`, and `INVALID` findings are advisory and never block integration/)
  assert.match(protocol, /Allow at most five substantive normal Implementation attempts per generation, and track review passes separately/)
  assert.match(protocol, /This is the only broad review/)
  assert.match(protocol, /first evidence-complete review in a generation uses `DISCOVERY`.*regardless of the Implementation attempt ordinal/)
  assert.match(protocol, /verify only open authorized blocker IDs/)
  assert.match(protocol, /Assign each `NEW` finding the next stable ID/)
  assert.match(protocol, /Dispatch it after every Reviewer response, including `APPROVE`, and once at attempt-budget exhaustion/)
  assert.match(protocol, /When attempt 5 ends without a reviewable frozen branch, dispatch Judge directly/)
  assert.match(protocol, /Judge sends valid findings outside the original task and repair delta to `leak\/`/)
  assert.match(protocol, /Never reopen broad discovery/)
  assert.match(protocol, /Resume or restack never resets the Implementation attempt count, review-pass count, Saver eligibility, or ledger/i)
  assert.match(protocol, /A `REVISE` containing only P2\/P3, `FOLLOWUP`, `INVALID`/)
  assert.match(protocol, /DECISION: DONE \| REPAIR \| SAVER \| NEEDS_INPUT \| BLOCKED/)
  assert.match(protocol, /<plan_dir>\/leak\/<source-plan-id>-<finding-id>-<slug>\.md/)
  assert.match(protocol, /conservative legacy fallback/)
  assert.match(protocol, /one substantive Saver attempt is unused/)
  assert.match(protocol, /never replans, self-approves, or receives another substantive attempt/)
  assert.match(protocol, /An explicit resume does not replenish the five normal Implementation attempts or the Saver attempt/)
  assert.match(protocol, /For transient capacity, do not increment any retry, interruption, clarification, implementation, or Saver bound/)
  ...
  assert.match(protocol, /at most two same-attempt non-capacity interruption restarts/)
  assert.match(protocol, /infrastructure capacity unavailable; recovery budget preserved/)
  assert.doesNotMatch(protocol, /at most two same-attempt interruption restarts/)
  assert.match(protocol, /git worktree lock --reason plan-herder:/)
  assert.match(protocol, /Serialize restacking, coordinator gates, Reviewer, Judge, and integration advancement through one transaction lane/i)
  assert.match(protocol, /Never resume the interrupted child conversation/)
  assert.match(protocol, /--workdir <absolute-worktree>/)
  assert.match(protocol, /Dirty, conflicted, or rebasing with no active owner: preserve exact worktree/)
  ...
  assert.match(orcaProtocol, /normal focused plan through Codex controller, Grok implementer, Pi\/Kimi reviewer, and Pi\/GPT Judge/)
  assert.match(orcaProtocol, /evidence-complete five-attempt ledger with separate review-pass accounting to Pi\/GPT Judge and a Judge-authorized one-shot recovery contract to Grok Saver/)
  ...
  for (const profile of [codexReviewer, claudeReviewer]) {
    ...
    assert.match(profile, /every review through Judge/)
    assert.match(profile, /first evidence-complete review regardless of Implementation attempt number/)
  }
  ...
  for (const profile of [codexJudge, claudeJudge]) {
    assert.match(profile, /Adjudicate every Reviewer response, including .*APPROVE/)
    assert.match(profile, /BLOCKING_IN_SCOPE.*NONBLOCKING_IN_SCOPE.*DEFERRED_OUT_OF_SCOPE.*REJECTED/)
    assert.match(profile, /When attempt 5 ends without a reviewable frozen branch/)
    assert.match(profile, /Never override a failed required gate/)
    assert.match(profile, /DECISION: DONE \| REPAIR \| SAVER \| NEEDS_INPUT \| BLOCKED/)
