...
This first installs the five native profiles in an isolated user-scoped Codex home, verifies that Multi-Agent V2 is enabled, and then exercises the full intent-to-plan pipeline against the fixture:
...
- Fire spawns exactly one persistent `plan_accountant`, dispatches every agent type with `fork_turns: "none"`, and never invokes the removed `codex exec` worker adapter. The installed native role set retains Saver for legacy resume compatibility, while the passing fresh fixture exercises Accountant, Implementer, and Reviewer and proves that both Judge and Saver are skipped.
