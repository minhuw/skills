...
python3 /path/to/skill-creator/scripts/quick_validate.py plugins/herder/skills/dashboard
...
The profile-registry tests cover deterministic compilation, host defaults, exact `offcut` orchestrator and child-role routing, `eclipse` resolution on Claude Code, opaque model identifiers, unique generated agent types, and stale-artifact checks. The Plans tests cover compiled shared-context snapshots, snapshot hashes, ignored legacy review-budget metadata, legacy shape warnings, dependency graphs, non-executable leak-directory isolation, lifecycle behavior, immutable SQLite run-profile binding, attempt accounting, unchanged README bytes, idempotent writes, legacy-ledger migration, integrity-preserving malformed-input failure, and rich per-plan completion reports. Configure tests cover legacy unqualified Codex alias generation, all supported Orca harness/mutability combinations, structural validation without model probes, conflict refusal, and recoverable backups. The Fire script tests cover selected-profile persistence and no-fallback dispatch failures, the persistent Accountant event contract, idempotent accounting, a separately reserved control slot, native worker identity, terminal and usage evidence, Orca runtime-profile validation and tracked dispatch construction, byte-stable checkout snapshots for tracked/index/untracked user state, Accountant gate isolation, namespaced collision preflight, the default five-slot role-mixed plan-worker pool, fully independent per-plan role pipelines, immediate slot backfilling, a final-only integration lock, one stable branch/worktree per plan, in-place checkpointed restacking, the shared canonical and legacy checkpoint-ref grammar with malformed refs failing closed, the deterministic six-round transition table, first-discovery review accounting, Judge escalation beginning at unresolved round 3, approval bypass, deferred leak records, stable relationship ledgers, repository-native linear history, private completion/checkpoint refs, and fail-closed cleanup. Dashboard tests build a real temporary Git repository with branches, a locked worker worktree, private completion evidence, a validated dependency graph, and SQLite attempts; they prove state derivation is byte-stable, exercise the loopback HTTP surface and security headers, and reject write methods. The assignment-bundle fixtures cover ignored and tracked plan directories, deterministic plan and integration-RUN materialization, branch and generation binding, source-path redaction, Git-status neutrality, read-only verification, stale snapshots, byte tampering, symlink escapes, missing ignore rules, and an isolated real conflicted rebase that accepts only exact Accountant-requested active-rebase evidence, rejects missing or changed metadata/refs/index/conflicts/worktree/assignment evidence, and returns to ordinary attached-branch verification after `git rebase --continue`. The namespace fixtures prove directory-derived names, explicit-name isolation, fresh-run collision refusal, resume validation, current and legacy checkpoint recognition, malformed checkpoint rejection, and parent/unknown branch protection. Cleanup fixtures prove native and Orca-owned removal, exact plan-branch recognition, current and legacy private checkpoint validation, preservation before finalization eligibility, absence of Herder metadata from new commit history, dry-run behavior, clean/unlocked `DONE` cleanup, default preservation of non-`DONE` evidence, explicit failed-evidence deletion, terminal finalization, and preservation of dirty, locked, unknown, proofless, integration, and log state.
...
This creates a real temporary Git repository and isolated `CODEX_HOME`, installs the current marketplace checkout through `codex plugin`, verifies all eight skills are cached, initializes an ignored `herder-plans/` backlog through the installed manager, records a SQLite attempt without changing README, queries its rich run report, validates the backlog, and runs the fixture's tests:
...
This first installs every bundled native profile in an isolated user-scoped Codex home, verifies that Multi-Agent V2 is enabled, and then exercises the full intent-to-plan pipeline against the fixture:

1. `$herder:grill <change>` investigates user intent, pauses for final confirmation, and creates a shape-clean semantically bounded plan graph without changing source code or execution-accounting data.
...
This mode adds two six-handler caller cohorts behind a compatibility normalizer, then asks Grill to plan a safe object-result migration. It verifies that Codex produces at least three focused nodes (bounded cohort migrations plus dependent cleanup), every node is at most 1,200 words, dependency edges are explicit, write-scope overlaps are ordered, and source remains unchanged.
...
- `.herder/execution.sqlite3` is unchanged; and
...
This high-cost mode creates a plan with Improve, narrows its declared path list while retaining required test and documentation work, and executes it through native Codex Multi-Agent V2. The isolated Codex configuration pins the root dispatcher to Sol/max, enables `multi_agent_v2`, reserves six child threads for the default five-worker pool plus Accountant, and gives the control plane only the workspace-write roots needed for disposable worktrees and Git metadata. The test installs the native profiles in a fresh session, then verifies:

- Fire resolves `eclipse`, spawns exactly one persistent `eclipse_plan_accountant`, dispatches its exact namespaced agent types with `fork_turns: "none"`, persists the profile hash/mapping, and never invokes the removed `codex exec` worker adapter. Legacy aliases remain installed for old runs, while the passing fresh fixture exercises Accountant, Implementer, and Reviewer and proves that both Judge and Saver are skipped.
- The Accountant runs Luna/max on Fast tier, Implementers run Luna/max on Fast tier, and reviewers run Sol/xhigh on Standard tier. The installed reviewer profile requests read-only; the transcript also records whether the controller's inherited runtime permission override superseded it, while Fire proves the reviewer left the plan branch unchanged.
- Every plan-worker transcript reports Multi-Agent V2 with one `NEW_TASK` envelope and no user-history messages. The Accountant reports one `NEW_TASK` plus repeated follow-up task envelopes on the same child thread, proving persistence without forking root history. Tool-call text is not treated as filesystem-containment evidence.
...
- Fire uses native `wait_agent` as a thirty-minute event-driven long poll rather than routine status polling. The root transcript contains only Accountant-authorized dispatch/wait transport, while the Accountant transcript owns `run-gate.mjs`, lifecycle accounting, and Git evidence.
...
The transcript files are `00-install.jsonl`, `01-improve.jsonl`, and `02-fire-run.jsonl`. The retained `reports/final-fire-report.md` contains Fire's user-facing result. `reports/native-spawn-evidence.json` records redacted namespaced routing arguments and root configuration; it records only that an encrypted task payload existed, never the payload itself. `reports/native-agent-evidence.json` records the persistent Accountant and worker roles, models, efforts, sandboxes, repository contexts, terminal classifications, follow-up evidence, and token telemetry extracted from persisted Codex sessions. Inspect those reports together with the fixture, integration worktree, and `herder-plans/README.md`, then delete the workspace when finished.
