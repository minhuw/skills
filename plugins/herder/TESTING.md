...
- The integrated diff exceeds the two-file expected target, stays within the five-file hard ceiling, and every implementation-discovered companion path is reported through `DISCOVERED_PATHS` for explicit review; Judge is required only if the plan reaches escalation.
