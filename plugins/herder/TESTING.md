...
1. `$herder:grill <change>` investigates user intent, pauses for final confirmation, and creates a shape-clean file-budgeted plan graph without changing source code or replacing the manager-generated usage ledger.
...
This high-cost mode creates a plan with Improve, narrows its expected path list and target to two files while retaining required test and documentation work, and executes it through native Codex Multi-Agent V2. The isolated Codex configuration pins the main scheduler to Sol/max, enables `multi_agent_v2`, and gives the coordinator only the workspace-write roots needed for disposable worktrees and Git metadata. The test installs the native profiles in a fresh session, then verifies:

- Fire dispatches `agent_type` with `fork_turns: "none"` and never invokes the removed `codex exec` worker adapter. The installed native role set includes Implementer, Reviewer, Judge, and Saver; the fixture exercises Implementer, Reviewer, and Judge while Saver remains unnecessary on a passing plan.
...
- The integrated diff exceeds the two-file expected target, stays within the five-file hard ceiling, and every implementation-discovered companion path is reported through `DISCOVERED_PATHS` for explicit review and judgment.
...
Before publishing, require all deterministic checks and the local installation smoke test. Run the general live test after changes to Improve output, the Plans protocol, or Fire's consumption of plan state. Run the live shaping test after changes to granularity, file targets, the fixed implementation contingency, shared context, or split rules. Run the targeted Grill test after changes to its interview, confirmation, or plan-editing contract. Run the targeted Validate test after changes to validation, repair boundaries, or Fire-readiness reporting. Run the high-cost native Fire execution mode when scheduling, namespace, worktree, model routing, usage capture, review, judging, recovery behavior, or discovered-path adjudication changes materially. Its fixture must intentionally declare a target/path set narrower than the expected test-and-documentation implementation, then assert that changed paths exceed the target, stay within `N+3`, and are reported through `DISCOVERED_PATHS`. Run the Orca heterogeneous mode after changes to cross-harness profile resolution, Orca worktree ownership, task dispatch/provenance, runtime recovery, or Orca cleanup.
